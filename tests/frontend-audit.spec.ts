...
test.describe('GUIDESOFT.AI Frontend Audit & User Flows', () => {
  test('1. Landing Page Hero, Logo, and Prompt Submission UI', async ({ page }) => {
    ...
    const pill = page.getByRole('button', { name: /SaaS Landing Page/i })
    await expect(pill).toBeVisible()

    // Click pill to verify prompt auto-fill
    await pill.click()
    await expect(promptInput).toHaveValue(/SaaS landing page/i)

    console.log('✅ Landing Page UI & Prompt Pills Verified Successfully')
  })
