...
import { Ollama, OpenRouter } from '@/components/logos'
...
const CODING_AGENTS = [
  ...
  { value: 'ollama', label: 'Ollama (Local)', icon: Ollama, isLogo: true },
  { value: 'openrouter', label: 'OpenRouter (OS)', icon: OpenRouter, isLogo: true },
] as const
...
const AGENT_MODELS = {
  ...
  ollama: [
    ...
    { value: 'ollama/llama3.3', label: 'Llama 3.3 70B' },
    { value: 'ollama/mistral-nemo', label: 'Mistral NeMo' },
    { value: 'ollama/codestral', label: 'Codestral 22B' },
  ],
  openrouter: [
    { value: 'deepseek/deepseek-r1', label: 'DeepSeek R1 (OpenSource)' },
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      <div className="text-center mb-8 space-y-4">
        {/* Centered GuideSoft Full Logo */}
        <div className="flex justify-center items-center pb-1">
          <img
            src="/guidesoft-full-logo-dark.png"
            alt="GuideSoft IT Solutions and Training Center"
            className="h-12 sm:h-16 md:h-20 w-auto object-contain max-w-full drop-shadow-md"
          />
        </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* leap.new Quick Prompt Ideas */}
        <div className="mt-6 flex flex-wrap items-center justify-center gap-2 text-xs">
          <span className="text-zinc-500 font-medium mr-1 text-[11px] uppercase tracking-wider">Try building:</span>
          {[
            '✨ SaaS Landing Page with Tailwind',
            '⚡ Next.js App with Auth & Database',
            '🎨 AI Dashboard with Charts & Dark Mode',
            '🛠️ REST API backend with Express & TypeScript',
          ].map((idea) => (
            <button
              key={idea}
              type="button"
              onClick={() => setPrompt(idea.replace(/^[^\s]+\s/, ''))}
              className="px-3 py-1.5 rounded-full bg-zinc-900/90 border border-zinc-800 text-zinc-300 hover:text-white hover:border-emerald-500/60 transition-all font-sans text-xs"
            >
              {idea}
            </button>
          ))}
